...
date: "March 14, 2021"
author: "Project Team 1: Grant Henderson, Leanna Kelly, Sidney Parel, and Trevor Vidano"
output:
  html_document:
    df_print: paged
...
The reported summary for the model relating $Y$, the value reported in the CDC Depression and Anxiety data, to $X$, number of new COVID-19 cases, shows a positive linear relationship that is significant at p value = 0.05, from this information we can reject the null hypothesis, $H_0$ which states that $B_2=0$. 

The model relating $Y$, the value reported in the CDC Depression and Anxiety data, to $X$ the nmber of new COVID-19 deaths, shows a positive linear relationship that is not significant at p value = 0.05, from this information we can accept the null hypothesis, $H_0$, which states that $B_2=0$. The rest of the analysis will focus on the relationship between new COVID-19 cases and the value reported in the CDC Depression and Anxiety data because that relationship was significant. 

The conclusion that can be drawn from this model is that an association exists between the number of confirmed COVID-19 cases and the frequency of indicators, or symptoms, of Depressive or Anxiety disorder. This answers our primary question of interest for this report. This conclusion, derived from the data from the WHO Coronavirus Dashboard and the HPS, also confirms the information presented by [Holmes and O'Connor (2020)](https://doi.org/10.1016/s2215-0366(20)30168-1) that there are effects from the COVID-19 pandemic on mental health. This information is also consistent with the Google Trends data that showed a significant increase in search terms related to poor mental well-being as associated with the global pandemic [(Brodeur et al., 2020)](https://doi.org/10.1016/j.jpubeco.2020.104346). The information here allows us to address our secondary question of interest of whether a specific population in the United States experiences the greatest frequency of symptoms. The populations examined here were race groups (Hispanic or Latino, Non-Hispanic Asian, Non-Hispanic black, Non-Hispanic white, and Non-Hispanic other races and multiple races) and age groups (18-29, 30-39, 40-49, 50-59, 60-69, 70-79, and 80 years and above). Examining levels of anxiety and depression with an ANOVA between race groups and age groups will show if the pandemic has affected the anxiety and depression levels of these race groups and age groups uniformly or disparately.
...
* Outliers
* Unequal variance in the error terms
* THe error terms not being distributed normally
...
To summarize the sensitivity analysis suggestion, our assumptions for both the linear model and the two ANOVA models appear to hold and nothing majorly concerning appears that would disqualify the results from the three models.
